...
def get_sendgrid_api_key():
    try:
        ...
        logging.info("Successfully retrieved SendGrid API key from Secret Manager")
        return api_key
    except Exception as e:
        logging.error(f"Error fetching SENDGRID_API_KEY from Secret Manager: {e}")
        return None
...
def send_email_notification(matched_articles):
    ...
    from_email = Email('dmegan@gmail.com', name='Dave Egan')  # Add your name for better authentication
    ...
    for to_email in to_emails:
        message = Mail(
            ...
            html_content=Content(MimeType.html, email_content)
        )
        ...
        try:
            ...
            if hasattr(e, 'body'):
                logger.error(f"Error response body: {e.body.decode()}")
            if hasattr(e, 'headers'):
                logger.error(f"Error response headers: {dict(e.headers)}")
